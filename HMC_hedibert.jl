...
# ╔═╡ 8ac71b1b-3873-4af6-8faa-50d82753ae53
begin
	using LinearAlgebra, Random, Distributions, Plots
	Random.seed!(123)
end

# ╔═╡ 33015b10-fa4f-11ec-134b-ed2d05b7a154
begin
	function post(theta)
	...
	function gradient(theta)
		[-theta[1]*theta[2]^2 + theta[1]-4, -theta[2]*theta[1]^2+theta[2]-4]
	end
	
	function leapfrog(theta, p, eps, iM)
...
# ╔═╡ 805a7582-1ae3-4354-9cec-9c44e948d94f
begin
	M      = I(2)
	iM     = M^-1
	tcM    = cholesky(M).U
	...
	N      = 10000
	niter  = burnin + N
	thetas = zeros(niter, 2)
end;

# ╔═╡ a5f2e171-4b43-422c-b9af-bdf06b8b8aff
for iter = 1:niter
	p      = tcM * rand(MvNormal(I(2)))
	theta1 = theta
	p1     = copy(p)
	for i = 1:L
		theta1, p1 = leapfrog(theta1,p1,eps,iM)
	end
	term1 = post(theta1) / post(theta)
	term2 = exp(-0.5 * p1'*iM*p1) / exp(-0.5* p'*iM*p)
	accept = min(1,term1 .* term2)
	if rand(Uniform()) < accept
		theta = theta1
		p     = - p1
	end
	thetas[iter,:]  = theta
...
# ╔═╡ e0ddbfe5-7227-4dd9-a9e0-42df758b2e4a
histogram(thetas[:,1])

# ╔═╡ f5211234-6c10-41e9-afd7-450ddea57fd2
plot(1:20000, thetas[:,1])
